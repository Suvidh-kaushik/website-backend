...
/**
 * Service to validate the impersonation request
 ...
 * @throws {Forbidden} If a pending or unfinished approved request exists.
 * @throws {Error} Logs and rethrows any error encountered during validation.
 */
export const validateImpersonationRequestService = async (
  ...
  try {
    ...
    if (
      isApprovedImpersonatedRequestPresent &&
      !isApprovedImpersonatedRequestPresent.isImpersonationFinished
    ) {
...
/**
 * Service which creates a new Impersonation Request after validating it
 * @param {CreateImpersonationRequestServiceBody} body - The data for the new impersonation request.
 * @returns {Promise<object>} The created impersonation request object.
 * @throws {NotFound} If the impersonated user does not exist.
 * @throws {Forbidden} If a conflicting request exists.
...
export const createImpersonationRequestService = async (
  ...
  try {
    ...
    await validateImpersonationRequestService(body.userId, body.impersonatedUserId);
